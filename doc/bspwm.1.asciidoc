...
FLAG := hidden|collapsed|sticky|private|locked|marked|urgent
...
[!](hidden|collapsed|sticky|private|locked|marked|urgent)::
...
collapsed::
	Descendants of an internal node occupy the same space.
...
*-I*, *--invert*::
    Swap children of a selected node
...
*-g*, *--flag* hidden|collapsed|sticky|private|locked|marked[=on|off]::
...
'node_flag <monitor_id> <desktop_id> <node_id> hidden|collapsed|sticky|private|locked|marked|urgent on|off'::
